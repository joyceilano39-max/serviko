...
const getServiceNames = (realServices: any) => {
  if (!realServices || !Array.isArray(realServices)) return [];
  return realServices.map((s: any) => s.name);
};
...
export default function HomePage() {
      ...
      navigator.geolocation.getCurrentPosition(
        pos => {
          fetch(`/api/artists?lat=${pos.coords.latitude}&lng=${pos.coords.longitude}`)
            .then(res => res.json())
            .then(data => { 
              // Filter: Only show artists with real services AND profile photo
              const realArtists = (data.artists || []).filter((a: Artist) => 
                a.real_services && Array.isArray(a.real_services) && a.real_services.length > 0 && a.profile_photo
              );
  ...
  const getSimilarArtists = (artist: Artist) => {
    const artistServices = getServiceNames(artist.real_services);
    return artists.filter(a => a.id !== artist.id && a.is_available &&
      getServiceNames(a.real_services).some(s => artistServices.some(as => s.toLowerCase().includes(as.toLowerCase().split(" ")[0]))));
  };

  const filteredArtists = artists.filter(a => {
    const serviceNames = getServiceNames(a.real_services);
    const matchesSearch = search === "" || a.name.toLowerCase().includes(search.toLowerCase()) ||
      serviceNames.some(s => s.toLowerCase().includes(search.toLowerCase()));
    const matchesCategory = selectedCategory === "" ||
      serviceNames.some(s => s.toLowerCase().includes(selectedCategory.toLowerCase()));
    ...
    <div style={{ minHeight: "100vh", background: "#f8f8f8", fontFamily: "Arial, sans-serif" }}>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 200 }}>
          <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "32px 24px", width: "100%", maxWidth: "600px" }}>
            <div style={{ textAlign: "center", marginBottom: "24px" }}>
              <div style={{ width: "48px", height: "48px", borderRadius: "50%", background: "#E61D72", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", fontWeight: 700, fontSize: "20px" }}>S</div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "20px" }}>
              <div onClick={() => {
                ...
                <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "#E61D72", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "12px", flexShrink: 0 }}>📍</div>
              ...
              <a href="/map-pin" style={{ display: "flex", alignItems: "center", gap: "10px", background: "#F5F3FF", borderRadius: "12px", padding: "14px 16px", cursor: "pointer", textDecoration: "none" }}>
                <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "#7C3AED", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "11px", flexShrink: 0 }}>🗺️</div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 100, padding: "24px" }}>
          <div style={{ background: "#fff", borderRadius: "24px", padding: "32px", maxWidth: "400px", width: "100%", textAlign: "center" }}>
            {!notified ? (
              ...
              <>
                <div style={{ width: "60px", height: "60px", borderRadius: "50%", background: "#F0FDF4", margin: "0 auto 16px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "28px" }}>✓</div>
              ...
              getSimilarArtists(similarArtist).map(artist => (
                <div key={artist.id} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "14px 0", borderBottom: "1px solid #f0f0f0" }}>
                  <div style={{ display: "flex", gap: "12px", alignItems: "center" }}>
                    <img src={artist.profile_photo} alt={artist.name}
      ...
      <div style={{ maxWidth: "1100px", margin: "0 auto", padding: "16px" }}>
        ...
        <div>
          ...
          {loading ? (
            ...
            <div style={{ textAlign: "center", padding: "48px", background: "#fff", borderRadius: "20px" }}>
              <p style={{ color: "#888", fontSize: "16px", marginBottom: "8px" }}>No artists available yet</p>
              <p style={{ color: "#aaa", fontSize: "13px" }}>Check back soon for new service providers in your area!</p>
            </div>
          ) : (
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(280px, 1fr))", gap: "16px" }}>
              {filteredArtists.map(artist => {
                const serviceNames = getServiceNames(artist.real_services);
                return (
                    ...
                    <div style={{ padding: "14px 16px" }}>
                      <div style={{ display: "flex", gap: "6px", flexWrap: "wrap", marginBottom: "10px" }}>
                        ...
                        {serviceNames.length > 3 && (
                          <span style={{ background: "#f0f0f0", color: "#888", padding: "3px 8px", borderRadius: "20px", fontSize: "10px" }}>+{serviceNames.length - 3}</span>
                        )}
                      </div>
                      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                        <div>
                          <p style={{ color: "#888", fontSize: "10px", margin: 0 }}>Starting at</p>
                          <p style={{ fontWeight: 900, color: "#E61D72", fontSize: "18px", margin: 0 }}>₱{artist.starting_price}</p>
        ...
        <div style={{ background: "linear-gradient(135deg, #7C3AED, #4F46E5)", borderRadius: "20px", padding: "24px", marginTop: "20px", color: "#fff", display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "80px" }}>
          ...
          <div style={{ width: "60px", height: "60px", borderRadius: "50%", background: "rgba(255,255,255,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 900, fontSize: "24px", color: "#fff" }}>💰</div>
        </div>
      </div>
      ...
      <div style={{ position: "fixed", bottom: 0, left: 0, right: 0, background: "#fff", padding: "10px 0", boxShadow: "0 -4px 20px rgba(0,0,0,0.08)", display: "flex", justifyContent: "space-around", zIndex: 50 }}>
        {[
          ...
          { label: "🗺️ Map", href: "/map-pin" },
          { label: "📍 Track", href: "/tracking" },
          { label: "👤 Profile", href: "/dashboard" },
        ].map(item => (
          <Link key={item.label} href={item.href} style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: "2px", textDecoration: "none", color: "#888", minWidth: "50px", fontSize: "10px", fontWeight: 600 }}>
            {item.label}
          </Link>
        ))}
      </div>
