...
const plans = [
  { name: "Basic", price: 499, color: "#888", popular: false, features: ["3 bookings per month", "Access to all services", "Email support", "Cash payment only"], cta: "Get Started" },
  { name: "Popular", price: 999, color: "#E61D72", popular: true, features: ["10 bookings per month", "Access to all services", "Priority support", "Cash & Card payment", "Book in advance", "Cancel anytime"], cta: "Get Popular" },
  { name: "Premium", price: 1999, color: "#7C3AED", popular: false, features: ["Unlimited bookings", "Access to all services", "24/7 VIP support", "Cash & Card payment", "Book in advance", "Cancel anytime", "Dedicated laborer", "Home service priority"], cta: "Go Premium" },
];
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: "24px" }}>
          {plans.map((plan) => (
            <div key={plan.name} style={{ borderRadius: "24px", padding: "32px", border: `2px solid ${plan.popular ? "#E61D72" : "#FFE4F0"}`, background: plan.popular ? "#FFF0F6" : "#fff", position: "relative", boxShadow: plan.popular ? "0 8px 32px rgba(230,29,114,0.15)" : "0 2px 12px rgba(0,0,0,0.06)" }}>
              {plan.popular && (<div style={{ position: "absolute", top: "-14px", left: "50%", transform: "translateX(-50%)", background: "#E61D72", color: "#fff", padding: "4px 20px", borderRadius: "20px", fontSize: "13px", fontWeight: 700 }}>Most Popular</div>)}
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "12px", marginBottom: "32px" }}>
                {plan.features.map((f) => (
                  <div key={f} style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "14px" }}>
                    <span style={{ color: "#E61D72", fontWeight: 700 }}>✓</span>
                    <span style={{ color: "#555" }}>{f}</span>
                  </div>
                ))}
              </div>
              <Link href="/register" style={{ display: "block", textAlign: "center", padding: "14px", borderRadius: "12px", textDecoration: "none", background: plan.popular ? "#E61D72" : "#fff", color: plan.popular ? "#fff" : "#E61D72", border: "2px solid #E61D72", fontWeight: 700, fontSize: "15px" }}>{plan.cta}</Link>
            </div>
          ))}
        </div>
      </div>
