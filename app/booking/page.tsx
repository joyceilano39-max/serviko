...
const savedAddresses = [
  { id: 1, icon: "Home", label: "Home", address: "FCM North Fairview, Quezon City", isDefault: true },
  { id: 2, icon: "Work", label: "Work", address: "Ayala Ave, Makati City", isDefault: false },
  { id: 3, icon: "Parents", label: "Parents", address: "San Antonio, Quezon City", isDefault: false },
];
...
function BookingContent() {
  ...
  const handlePay = async () => {
    ...
    try {
      // Save booking data
      localStorage.setItem("serviko_booking", JSON.stringify({
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFF0F6", fontFamily: "Arial, sans-serif" }}>
      ...
      <div style={{ background: "#fff", padding: "14px 24px", display: "flex", alignItems: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", overflowX: "auto" }}>
        {(artistIdParam
          ? [{ n: 2, label: "Services" }, { n: 3, label: "Schedule & Pay" }]
          : [{ n: 1, label: "Artist" }, { n: 2, label: "Services" }, { n: 3, label: "Schedule & Pay" }]
        ).map((s, i, arr) => (
      ...
      <div style={{ maxWidth: "900px", margin: "0 auto", padding: "20px", display: "grid", gridTemplateColumns: "1fr 280px", gap: "20px" }}>
        <div>
                  ...
                  {filteredArtists.map(artist => (
                    <div key={artist.id} onClick={() => setSelectedArtist(artist)}
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
                        <div style={{ width: "50px", height: "50px", borderRadius: "50%", background: "#FFF0F6", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "13px", fontWeight: 700, color: "#E61D72", flexShrink: 0 }}>
                        ...
                        <div style={{ flex: 1 }}>
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <div>
                              ...
                              <p style={{ color: "#888", fontSize: "12px", margin: "0 0 4px" }}>Location: {artist.location}</p>
                            ...
                            <div style={{ textAlign: "right" }}>
                              ...
                              {selectedArtist?.id === artist.id && <p style={{ color: "#E61D72", fontSize: "18px", margin: "4px 0 0" }}>Selected</p>}
                            </div>
                          </div>
                        </div>
                      </div>
          ...
          {step === 2 && selectedArtist && (
            <div>
              <div style={{ background: "#FFF0F6", borderRadius: "12px", padding: "12px 16px", marginBottom: "16px", display: "flex", alignItems: "center", gap: "12px" }}>
                <div style={{ width: "40px", height: "40px", borderRadius: "50%", background: "#E61D72", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "12px", fontWeight: 700 }}>
                  {getArtistEmoji(selectedArtist.services)}
                </div>
                <div>
          ...
          {/* STEP 3: Schedule + Voucher + Payment ALL IN ONE */}
          {step === 3 && (
            <div style={{ display: "flex", flexDirection: "column", gap: "16px" }}>

              {/* Schedule */}
              <div style={{ background: "#fff", borderRadius: "18px", padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                ...
                <div style={{ marginBottom: "14px" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "8px" }}>
                    ...
                    <button onClick={() => setShowAddressModal(true)}
                      style={{ background: "none", border: "none", color: "#E61D72", fontWeight: 600, fontSize: "12px", cursor: "pointer" }}>Change</button>
                  </div>
                  <div style={{ background: "#FFF0F6", borderRadius: "12px", padding: "12px 14px", border: "2px solid #E61D72" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                      <div style={{ width: "32px", height: "32px", borderRadius: "50%", background: "#E61D72", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "11px", fontWeight: 700 }}>
                  ...
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "8px" }}>
                    {Object.entries(transportFees).map(([range, fee]) => (
                      <div key={range} onClick={() => setDistance(range)}
                        ...
                        <p style={{ color: "#E61D72", fontWeight: 700, margin: 0, fontSize: "12px" }}>+P{fee}</p>
                      </div>
                    ))}
                  </div>
                </div>
              ...
              {/* Voucher */}
              <div style={{ background: "#fff", borderRadius: "18px", padding: "20px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                <h3 style={{ fontWeight: 700, margin: "0 0 12px", fontSize: "15px" }}>Voucher / Promo Code</h3>
                {appliedVoucher ? (
                  <div style={{ background: "#F0FDF4", borderRadius: "10px", padding: "12px", display: "flex", justifyContent: "space-between", alignItems: "center", border: "1px solid #86EFAC" }}>
                    <div>
                      <p style={{ fontWeight: 700, color: "#22c55e", margin: "0 0 2px", fontSize: "13px" }}>Applied: {appliedVoucher.code}</p>
                      <p style={{ color: "#555", fontSize: "12px", margin: 0 }}>You save P{appliedVoucher.discount}!</p>
                    </div>
                  ...
                  <>
                    <div style={{ display: "flex", gap: "8px", marginBottom: "8px" }}>
                      <input value={voucherCode} onChange={e => setVoucherCode(e.target.value.toUpperCase())} placeholder="Enter voucher or referral code"
                        style={{ flex: 1, padding: "11px", borderRadius: "10px", border: "1px solid #FFD6E7", fontSize: "13px", fontWeight: 600 }} />
                      <button onClick={applyVoucher} style={{ background: "#E61D72", color: "#fff", padding: "11px 16px", borderRadius: "10px", border: "none", fontWeight: 700, cursor: "pointer" }}>Apply</button>
                    </div>
              ...
              <div style={{ display: "flex", gap: "10px" }}>
                ...
                <button onClick={handlePay} disabled={payLoading || !name || !email}
                  ...
                  {payLoading ? "Processing..." : `Pay P${total}`}
                </button>
              </div>
          ...
          <div style={{ background: "#fff", borderRadius: "18px", padding: "18px", boxShadow: "0 4px 16px rgba(0,0,0,0.08)" }}>
            ...
            {selectedArtist && (
              <div style={{ background: "#FFF0F6", borderRadius: "8px", padding: "8px 10px", marginBottom: "10px" }}>
                <p style={{ fontWeight: 700, margin: "0 0 2px", fontSize: "12px", color: "#E61D72" }}>{selectedArtist.name}</p>
            ...
            {allServices.length > 0 && (
              <div style={{ borderTop: "1px solid #FFE4F0", paddingTop: "8px" }}>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", marginBottom: "3px" }}>
                  <span style={{ color: "#888" }}>Subtotal</span><span>P{subtotal}</span>
                </div>
                <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", marginBottom: "3px" }}>
                  <span style={{ color: "#888" }}>Transport</span><span>P{transport}</span>
                </div>
                {discount > 0 && (
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "12px", marginBottom: "3px" }}>
                ...
                <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 900, fontSize: "16px", borderTop: "1px solid #FFE4F0", paddingTop: "8px", marginTop: "4px" }}>
                  <span>Total</span><span style={{ color: "#E61D72" }}>P{total}</span>
                </div>
              </div>
            )}
            {date && (
              <div style={{ background: "#FFF0F6", borderRadius: "8px", padding: "8px", marginTop: "8px", fontSize: "11px" }}>
                <p style={{ margin: "0 0 2px", fontWeight: 600 }}>Date: {date} at {time}</p>
                <p style={{ margin: 0, color: "#888" }}>Address: {selectedAddress.address}</p>
              </div>
            )}
          </div>
        </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "28px 24px", width: "100%", maxWidth: "600px" }}>
            <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "16px" }}>
              ...
              <button onClick={() => setShowAddressModal(false)} style={{ background: "none", border: "none", fontSize: "22px", cursor: "pointer" }}>X</button>
            </div>
            {savedAddresses.map(addr => (
              <div key={addr.id} onClick={() => { setSelectedAddress(addr); setShowAddressModal(false); }}
                ...
                <div style={{ width: "36px", height: "36px", borderRadius: "50%", background: "#E61D72", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "12px" }}>
                  {addr.label[0]}
                </div>
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", gap: "6px", alignItems: "center" }}>
                ...
                {selectedAddress.id === addr.id && <span style={{ color: "#E61D72", fontWeight: 700 }}>Selected</span>}
              </div>
            ))}
...
export default function BookingPage() {
  return (
    <Suspense fallback={
      <div style={{ display: "flex", alignItems: "center", justifyContent: "center", minHeight: "100vh", background: "#FFF0F6" }}>
        <p style={{ color: "#E61D72", fontWeight: 700, fontSize: "18px" }}>Loading...</p>
      </div>
    }>
      <BookingContent />
    </Suspense>
