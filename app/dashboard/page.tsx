...
export default function CustomerDashboardPage() {
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case "declined":
        return { bg: "#FEF2F2", color: "#f87171", label: "Declined" };
      default:
        return { bg: "#f8f8f8", color: "#888", label: status };
    }
  };
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#FFF0F6", fontFamily: "Arial, sans-serif" }}>
      <div style={{ background: "#fff", borderBottom: "1px solid #E5E7EB", padding: "12px 24px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h1 style={{ fontSize: "20px", fontWeight: 700, margin: 0 }}>My Dashboard</h1>
        <UserButton />
      </div>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
          <div>
            ...
            <p style={{ opacity: 0.8, fontSize: "13px", margin: 0 }}>
              {upcoming.length > 0
                ? `${upcoming.length} upcoming booking(s)`
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: "12px", padding: "16px 20px" }}>
        {[
          ...
          <div key={stat.label} style={{ background: stat.bg, borderRadius: "14px", padding: "14px", textAlign: "center" }}>
            <p style={{ fontWeight: 900, fontSize: "24px", color: stat.color, margin: "0 0 2px" }}>
              {stat.value}
            </p>
        ...
        <div style={{ padding: "0 20px 16px" }}>
          ...
          <div style={{ display: "flex", gap: "10px", overflowX: "auto", paddingBottom: "8px" }}>
            {vouchers.slice(0, 3).map((v, i) => (
              <div key={v.id} style={{ background: "#fff", borderRadius: "12px", padding: "12px", minWidth: "200px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", borderLeft: `4px solid ${getVoucherColor(i)}` }}>
                <p style={{ fontWeight: 900, fontSize: "14px", color: getVoucherColor(i), margin: "0 0 4px" }}>
                  {v.code}
                </p>
                <p style={{ color: "#555", fontSize: "12px", margin: 0 }}>{v.description}</p>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ padding: "0 20px 16px" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: "10px" }}>
            {artists.slice(0, 4).map((artist) => (
              <Link key={artist.id} href={`/artist/${artist.id}`} style={{ background: "#fff", borderRadius: "12px", padding: "12px", textDecoration: "none", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                <div style={{ width: "100%", height: "80px", borderRadius: "8px", background: "#F5F3FF", marginBottom: "8px", display: "flex", alignItems: "center", justifyContent: "center", fontSize: "32px" }}>
                  {artist.photo ? (
                ...
                <p style={{ color: "#888", fontSize: "11px", margin: 0 }}>{artist.service}</p>
              </Link>
            ))}
          </div>
        </div>
      ...
      <div style={{ background: "#fff", display: "flex", gap: "4px", padding: "12px 20px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", overflowX: "auto" }}>
        {[
          ...
          <button key={tab.id} onClick={() => setActiveTab(tab.id as TabType)} style={{ padding: "8px 16px", borderRadius: "20px", border: "none", cursor: "pointer", fontWeight: 600, fontSize: "12px", whiteSpace: "nowrap", background: activeTab === tab.id ? "#E61D72" : "#f0f0f0", color: activeTab === tab.id ? "#fff" : "#555" }}>
            {tab.label}
          </button>
        ))}
      </div>
          ...
          <div>
            <Link href="/booking" style={{ display: "block", background: "linear-gradient(135deg, #E61D72, #7C3AED)", color: "#fff", padding: "16px 20px", borderRadius: "16px", textDecoration: "none", marginBottom: "16px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div>
                  <p style={{ fontWeight: 900, fontSize: "16px", margin: "0 0 4px" }}>
                    Book a Service
                  </p>
                  <p style={{ opacity: 0.8, fontSize: "12px", margin: 0 }}>
                    Find available artists near you
                  </p>
                </div>
                <div style={{ background: "rgba(255,255,255,0.2)", padding: "10px 16px", borderRadius: "20px", fontWeight: 700, fontSize: "13px" }}>
                  Book Now
                </div>
              </div>
            </Link>
            ...
            {loading ? (
              ...
              <div style={{ background: "#fff", borderRadius: "20px", padding: "48px", textAlign: "center", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
                ...
                <Link href="/booking" style={{ background: "#E61D72", color: "#fff", padding: "12px 24px", borderRadius: "20px", textDecoration: "none", fontWeight: 700, fontSize: "14px" }}>
                  Book Now
                </Link>
              </div>
            ) : (
                    ...
                    <div key={booking.id} onClick={() => setSelectedBooking(booking)} style={{ background: "#fff", borderRadius: "16px", padding: "16px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", cursor: "pointer", border: booking.status === "confirmed" ? "2px solid #3b82f6" : "2px solid transparent" }}>
                      <div style={{ display: "flex", justifyContent: "space-between", marginBottom: "10px" }}>
                        <div>
                          ...
                          <p style={{ color: "#888", fontSize: "12px", margin: "0 0 2px" }}>
                      ...
                      {booking.status === "completed" && (
                        <Link href="/reviews" onClick={(e) => e.stopPropagation()} style={{ display: "block", background: "#FFF0F6", color: "#E61D72", padding: "8px", borderRadius: "10px", textDecoration: "none", fontWeight: 600, fontSize: "12px", textAlign: "center", marginTop: "8px" }}>
                          Leave a Review
                        </Link>
                      )}
                    </div>
        ...
        {activeTab === "profile" && (
          <div style={{ background: "#fff", borderRadius: "20px", padding: "24px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)" }}>
            <div style={{ textAlign: "center", marginBottom: "24px" }}>
              <div style={{ width: "80px", height: "80px", borderRadius: "50%", background: "#E61D72", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", margin: "0 auto 12px", fontSize: "28px", fontWeight: 700 }}>
                {user?.firstName?.[0] || "C"}
              </div>
              <p style={{ fontWeight: 700, fontSize: "18px", margin: "0 0 4px" }}>
        ...
        {activeTab === "vouchers" && (
          <div>
            ...
            {vouchers.length === 0 ? (
              ...
              vouchers.map((v, i) => (
                <div key={v.id} style={{ background: "#fff", borderRadius: "16px", padding: "16px", marginBottom: "10px", boxShadow: "0 2px 8px rgba(0,0,0,0.06)", borderLeft: `4px solid ${getVoucherColor(i)}` }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <div>
                      <p style={{ fontWeight: 900, fontSize: "16px", color: getVoucherColor(i), margin: "0 0 4px" }}>
                        {v.code}
                      </p>
                      <p style={{ color: "#555", fontSize: "13px", margin: "0 0 2px" }}>
                        {v.description}
                      </p>
                    ...
                    }} style={{ background: getVoucherColor(i), color: "#fff", border: "none", padding: "8px 14px", borderRadius: "20px", cursor: "pointer", fontWeight: 600, fontSize: "12px" }}>
                      Copy
                    </button>
                  </div>
                </div>
        ...
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "flex-end", justifyContent: "center", zIndex: 100 }}>
          <div style={{ background: "#fff", borderRadius: "24px 24px 0 0", padding: "28px 24px", width: "100%", maxWidth: "600px", maxHeight: "80vh", overflowY: "auto" }}>
            ...
            {[
              {
                label: "Artist",
                val: selectedBooking.artist_name || `Artist #${selectedBooking.artist_id}`,
              },
            ...
            <div style={{ display: "flex", gap: "10px", marginTop: "20px" }}>
              ...
              {selectedBooking.status === "completed" && (
                <Link href="/reviews" style={{ flex: 1, background: "#E61D72", color: "#fff", padding: "12px", borderRadius: "12px", textDecoration: "none", fontWeight: 700, textAlign: "center" }}>
                  Leave Review
                </Link>
              )}
              <button onClick={() => setSelectedBooking(null)} style={{ flex: 1, background: "#f0f0f0", color: "#555", border: "none", padding: "12px", borderRadius: "12px", fontWeight: 700, cursor: "pointer" }}>
                Close
              </button>
            </div>
          </div>
